...
const GameMenu = () => {
  ...
  useEffect(() => {
    const storedInProgress =
      sessionStorage.getItem("actionInProgress_player3") === "true";
    const storedLockKey = sessionStorage.getItem("actionLockKey_player3");
    const storedLockTs = Number(
      sessionStorage.getItem("actionLockTs_player3") || "0"
    );
    ...
    if (storedInProgress && (!storedLockKey || isExpired)) {
      sessionStorage.removeItem("actionInProgress_player3");
      sessionStorage.removeItem("actionLockKey_player3");
      sessionStorage.removeItem("actionLockTs_player3");
  ...
  useEffect(() => {
    if (actionInProgress && actionLockKey) {
      sessionStorage.setItem("actionInProgress_player3", "true");
      sessionStorage.setItem("actionLockKey_player3", actionLockKey);
      if (!sessionStorage.getItem("actionLockTs_player3")) {
      ...
      ws.onmessage = (event) => {
        ...
        switch (data.action) {
          ...
          case "game_evaluated":
            ...
            setResultPopupDismissed(false);
            // Get player3's result
            const player3Data = data.game_state?.players?.player3;
            if (player3Data) {
              const mainHandResult = player3Data.hands?.[0]?.result;
              const split1Result = player3Data.split1?.[0]?.result;
              const split2Result = player3Data.split2?.[0]?.result;
          ...
          case "manual_make_default":
            // Check if this action affects player3
            if (data.player_id === "player3") {
  ...
  useEffect(() => {
    ...
    if (
      ...
      const player3Data = gameState.players.player3;
      const mainHandResult = player3Data.hands?.[0]?.result;
      const split1Result = player3Data.split1?.[0]?.result;
      const split2Result = player3Data.split2?.[0]?.result;
  ...
  useEffect(() => {
    if (!gameState?.players?.player3?.hands?.[0]?.cards) return;

    const mainHandCards = gameState.players.player3.hands[0].cards;
                        ...
                        <div className="space-y-6">
                          ...
                          >
                            ...
                            <div className="flex justify-center items-center gap-4 mb-4">
                              {gameState.players.player3.hands[0]?.cards?.map(
                                (card: string, index: number) => (
                                  <div
                                    key={index}
                            ...
                            {gameState.mode !== "manual" && (
                              <div className="absolute top-[90%] left-[43%] text-center">
                                <div className="inline-block bg-[#911606] px-3 py-1 rounded-full text-sm font-bold text-[#d4af37] border-2 border-[#d4af37]">
                                  Total:{" "}
                                  {gameState.players.player3.hands[0]?.total ??
                                    0}
                                </div>
                              </div>
                            )}
                          ...
                          {gameState.mode === "manual" &&
                            gameState.players.player3.split1_status === 1 && (
                              <div className="mt-4">
                                <div
                                  className={`rounded-xl p-4 relative ${getHandBoxColor(
                                    isHandSelected(
                          ...
                          {gameState.mode === "manual" &&
                            gameState.players.player3.split2_status === 1 && (
                              <div className="mt-4">
                                <div
                                  className={`rounded-xl p-4 relative ${getHandBoxColor(
                                    isHandSelected(
                            ...
                            0 && (
                              <div className="mt-4">
                                ...
                                >
                                  ...
                                  <div className="flex justify-center items-center gap-4 mb-4">
                                    {gameState.players.player3.split1[0].cards.map(
                                      (card, index) => (
                                        <div
                                          key={index}
                                          className="relative w-36 h-52 transform hover:scale-110 transition-transform duration-200 group"
                            ...
                            0 && (
                              <div className="mt-4">
                                ...
                                >
                                  ...
                                  <div className="flex justify-center items-center gap-4 mb-4">
                                    {gameState.players.player3.split2[0].cards.map(
                                      (card, index) => (
                                        <div
                                          key={index}
                                          className="relative w-36 h-52 transform hover:scale-110 transition-transform duration-200 group"
              ...
              <div className="bg-gradient-to-br from-green-900 via-green-800 to-emerald-900 border-4 border-yellow-400 text-white p-8 rounded-3xl shadow-2xl max-w-lg w-full mx-4 relative overflow-hidden">
                ...
                <div className="text-center relative z-10">
                  ...
                  <div className="mb-6 space-y-3">
                    {(() => {
                      const player3Data = gameState?.players?.player3;
                      if (!player3Data) return null;
                      const handResults = [
                        {
                          label: "Main Hand",
                          result: player3Data.hands?.[0]?.result,
                          icon: "🎰",
                        },
                        {
                          label: "Split 1",
                          result: player3Data.split1?.[0]?.result,
                          icon: "🃏",
                        },
                        {
                          label: "Split 2",
                          result: player3Data.split2?.[0]?.result,
                          icon: "🎯",
                        },
                      ];
