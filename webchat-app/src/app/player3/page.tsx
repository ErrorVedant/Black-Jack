...
const GameMenu = () => {
  ...
  const player3Active = gameState?.players?.player3?.status === 1;
      ...
      {player3Active ? (
        <div className="min-h-screen bg-gradient-to-br from-gray-900 to-black text-white p-8">
          ...
          <div className="max-w-7xl mx-auto flex flex-col gap-6 relative z-10">
            ...
            {/* Player 3 - Large Panel */}
            {gameState?.players?.player3 && (
              <div className="w-full">
                <div
                  className={`p-6 rounded-xl transition-all duration-300 ${isHandSelected(gameState, "player3", 0, 0)
                  ...
                  onClick={(e) => {
                    e.stopPropagation()
                    if (gameState.players.player3.status === 1) {
                      handlePlayerClick("player3")
                    }
                  }}
                >
                  <div className="flex flex-col space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-4">
                        <div
                          className={`w-5 h-5 rounded-full shadow-lg ${isHandSelected(gameState, "player3", 0, 0)
                        ...
                        />
                        <div>
                          <div className={`text-2xl font-bold ${isHandSelected(gameState, "player3", 0, 0) ? "text-gray-900" : "text-white"}`}>
                            Player 3
                          </div>
                          <div className={`text-base ${isHandSelected(gameState, "player3", 0, 0) ? "text-gray-700" : "opacity-75"}`}>
                            {isHandSelected(gameState, "player3", 0, 0) ? "Current Hand" : gameState.players.player3.status === 1 ? "Active" : "Inactive"}
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    {gameState.players.player3.status === 1 && (
                      <div className="space-y-6">
                        ...
                        <div className={`rounded-xl p-4 ${
                          isHandSelected(gameState, "player3", 0, 0) && gameState?.current_player === "player3"
                            ? "bg-yellow-300 border-2 border-yellow-500"
                            : "bg-black/20"
                        }`}>
                          <div className="flex items-center justify-between mb-3">
                            <div className={`text-lg font-medium ${isHandSelected(gameState, "player3", 0, 0) ? "text-gray-900" : "text-white"}`}>
                              Main Hand
                            </div>
                          </div>
                          <div className="flex space-x-3 mb-3">
                            {gameState.players.player3.hands[0]?.cards?.map((card: string, index: number) => (
                            ...
                            {[...Array(Math.max(0, 2 - (gameState.players.player3.hands[0]?.cards?.length ?? 0)))].map((_, index) => (
                              <div
                                key={`empty-${index}`}
                                className={`w-16 h-24 border-2 border-dashed rounded-lg ${isHandSelected(gameState, "player3", 0, 0)
                          ...
                          <div className="mt-3 flex items-center justify-between">
                            <span
                              className={`text-xl font-bold ${(gameState.players.player3.hands[0]?.total ?? 0) > 21
                                  ? "text-red-500"
                            ...
                            >
                              {gameState.players.player3.hands[0]?.total ?? 0}
                            </span>
                            <div className="flex space-x-3">
                              {isHandSelected(gameState, "player3", 0, 0) && gameState?.current_player === "player3" &&
                                gameState.players.player3.hands[0]?.cards?.length === 2 &&
                                canSplit(gameState.players.player3.hands[0].cards) &&
                                gameState.players.player3.hands[0].status === "playing" && (
                                <button
                                  onClick={() => sendWebSocketMessage({ action: "split_player_auto", player_id: "player3" })}
                          ...
                          {isHandSelected(gameState, "player3", 0, 0) && gameState?.current_player === "player3" && (
                            <div className="flex gap-2 flex-wrap mt-2">
                              {gameState?.dealer?.cards?.[0]?.[0] === "A" && !insuranceState[`player3_0_0`] && !gameState.players.player3.hands[0].insurence && (
                                <button
                                  onClick={() => handleInsurance("player3", 0, 0)}
                              ...
                              <button
                                onClick={() => { sendWebSocketMessage({ action: "hit_player", player_id: "player3", hand_index: 0 }); clearInsuranceForHand("player3", 0, 0); }}
                                className="px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors"
                              >
                                Hit
                              </button>
                              <button
                                onClick={() => { sendWebSocketMessage({ action: "double_player", player_id: "player3", hand_index: 0 }); clearInsuranceForHand("player3", 0, 0); }}
                                className="px-4 py-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition-colors"
                              >
                                Double
                              </button>
                              <button
                                onClick={() => { sendWebSocketMessage({ action: "next_turn", player_id: "player3", hand_index: 0 }); clearInsuranceForHand("player3", 0, 0); }}
                        ...
                        {/* Split1 Hand */}
                        {gameState.players.player3.split1[0]?.cards?.length > 0 && (
                          <div className="mt-4">
                            <div className={`rounded-xl p-4 ${
                              isHandSelected(gameState, "player3", 0, 1) && gameState?.current_player === "player3"
                                ? "bg-yellow-300 border-2 border-yellow-500"
                                : "bg-black/20"
                            }`}>
                              <div className="flex items-center justify-between mb-3">
                                <div className={`text-lg font-medium ${isHandSelected(gameState, "player3", 0, 1) ? "text-gray-900" : "text-white"}`}>
                                  Split Hand 1
                                </div>
                              </div>
                              <div className="flex space-x-3 mb-3">
                                {gameState.players.player3.split1[0].cards.map((card, index) => (
                                  <div key={index} className="relative w-16 h-24 transform hover:scale-110 transition-transform duration-200 group">
                                    <img
                                      src={`/cards/${card}.png`}
                                      alt={card}
                                ...
                                {[...Array(Math.max(0, 2 - (gameState.players.player3.split1[0].cards.length ?? 0)))].map((_, index) => (
                                  <div
                                    key={`empty-${index}`}
                                    className={`w-16 h-24 border-2 border-dashed rounded-lg ${isHandSelected(gameState, "player3", 0, 1)
                                        ? "border-yellow-400/50 bg-yellow-500/10"
                                        : "border-gray-400 bg-gray-800/50"
                                      }`}
                                  />
                              ...
                              <div className="mt-3 flex items-center justify-between">
                                <span
                                  className={`text-xl font-bold ${(gameState.players.player3.split1[0].total ?? 0) > 21
                                      ? "text-red-500"
                                      : isHandSelected(gameState, "player3", 0, 1)
                                        ? "text-gray-900"
                                        : "text-blue-400"
                                    }`}
                                >
                                  {gameState.players.player3.split1[0].total ?? 0}
                                </span>
                                <div className="flex space-x-3">
                                  {isHandSelected(gameState, "player3", 0, 1) && gameState?.current_player === "player3" &&
                                    gameState.players.player3.split1[0]?.cards?.length === 2 &&
                                    canSplit(gameState.players.player3.split1[0].cards) &&
                                    gameState.players.player3.split1[0].status === "playing" && (
                                    <button
                                      onClick={() => sendWebSocketMessage({ action: "split_player_auto", player_id: "player3" })}
                                      className="px-4 py-2 bg-purple-500 text-white rounded-lg hover:bg-purple-600 transition-colors"
                                    >
                              ...
                              {isHandSelected(gameState, "player3", 0, 1) && gameState?.current_player === "player3" && (
                                <div className="flex gap-2 flex-wrap mt-2">
                                  {gameState?.dealer?.cards?.[0]?.[0] === "A" && !insuranceState[`player3_0_1`] && !gameState.players.player3.split1[0].insurence && (
                                    <button
                                      onClick={() => handleInsurance("player3", 0, 1)}
                                      className="px-4 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition-colors"
                                    >
                                      Insurance
                                    </button>
                                  ...
                                  <button
                                    onClick={() => { sendWebSocketMessage({ action: "hit_player", player_id: "player3", hand_index: 0 }); clearInsuranceForHand("player3", 0, 1); }}
                                    className="px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors"
                                  >
                                    Hit
                                  </button>
                                  <button
                                    onClick={() => { sendWebSocketMessage({ action: "double_player", player_id: "player3", hand_index: 0 }); clearInsuranceForHand("player3", 0, 1); }}
                                    className="px-4 py-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition-colors"
                                  >
                                    Double
                                  </button>
                                  <button
                                    onClick={() => { sendWebSocketMessage({ action: "next_turn", player_id: "player3", hand_index: 0 }); clearInsuranceForHand("player3", 0, 1); }}
                                    className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors"
                                  >
                                    Stand
                                  </button>
                        ...
                        {/* Split2 Hand */}
                        {gameState.players.player3.split2[0]?.cards?.length > 0 && (
                          <div className="mt-4">
                            <div className={`rounded-xl p-4 ${
                              isHandSelected(gameState, "player3", 0, 2) && gameState?.current_player === "player3"
                                ? "bg-yellow-300 border-2 border-yellow-500"
                                : "bg-black/20"
                            }`}>
                              <div className="flex items-center justify-between mb-3">
                                <div className={`text-lg font-medium ${isHandSelected(gameState, "player3", 0, 2) ? "text-gray-900" : "text-white"}`}>
                                  Split Hand 2
                                </div>
                              </div>
                              <div className="flex space-x-3 mb-3">
                                {gameState.players.player3.split2[0].cards.map((card, index) => (
                                  <div key={index} className="relative w-16 h-24 transform hover:scale-110 transition-transform duration-200 group">
                                    <img
                                      src={`/cards/${card}.png`}
                                      alt={card}
                                ...
                                {[...Array(Math.max(0, 2 - (gameState.players.player3.split2[0].cards.length ?? 0)))].map((_, index) => (
                                  <div
                                    key={`empty-${index}`}
                                    className={`w-16 h-24 border-2 border-dashed rounded-lg ${isHandSelected(gameState, "player3", 0, 2)
                                        ? "border-yellow-400/50 bg-yellow-500/10"
                                        : "border-gray-400 bg-gray-800/50"
                                      }`}
                                  />
                              ...
                              <div className="mt-3 flex items-center justify-between">
                                <span
                                  className={`text-xl font-bold ${(gameState.players.player3.split2[0].total ?? 0) > 21
                                      ? "text-red-500"
                                      : isHandSelected(gameState, "player3", 0, 2)
                                        ? "text-gray-900"
                                        : "text-blue-400"
                                    }`}
                                >
                                  {gameState.players.player3.split2[0].total ?? 0}
                                </span>
                                <div className="flex space-x-3">
                                  {isHandSelected(gameState, "player3", 0, 2) && gameState?.current_player === "player3" &&
                                    gameState.players.player3.split2[0]?.cards?.length === 2 &&
                                    canSplit(gameState.players.player3.split2[0].cards) &&
                                    gameState.players.player3.split2[0].status === "playing" && (
                                    <button
                                      onClick={() => sendWebSocketMessage({ action: "split_player_auto", player_id: "player3" })}
                                      className="px-4 py-2 bg-purple-500 text-white rounded-lg hover:bg-purple-600 transition-colors"
                                    >
                              ...
                              {isHandSelected(gameState, "player3", 0, 2) && gameState?.current_player === "player3" && (
                                <div className="flex gap-2 flex-wrap mt-2">
                                  {gameState?.dealer?.cards?.[0]?.[0] === "A" && !insuranceState[`player3_0_2`] && !gameState.players.player3.split2[0].insurence && (
                                    <button
                                      onClick={() => handleInsurance("player3", 0, 2)}
                                      className="px-4 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition-colors"
                                    >
                                      Insurance
                                    </button>
                                  ...
                                  <button
                                    onClick={() => { sendWebSocketMessage({ action: "hit_player", player_id: "player3", hand_index: 0 }); clearInsuranceForHand("player3", 0, 2); }}
                                    className="px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors"
                                  >
                                    Hit
                                  </button>
                                  <button
                                    onClick={() => { sendWebSocketMessage({ action: "double_player", player_id: "player3", hand_index: 0 }); clearInsuranceForHand("player3", 0, 2); }}
                                    className="px-4 py-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition-colors"
                                  >
                                    Double
                                  </button>
                                  <button
                                    onClick={() => { sendWebSocketMessage({ action: "next_turn", player_id: "player3", hand_index: 0 }); clearInsuranceForHand("player3", 0, 2); }}
                                    className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors"
                                  >
                                    Stand
                                  </button>
                  ...
                  {/* Next Button */}
                  {gameState.players.player3.status === 1 && gameState?.current_turn === "player" && (
            ...
            <div className="grid grid-cols-3 gap-4 mt-6">
              {Object.entries(gameState?.players || {})
                .filter(([playerId]) => playerId !== "player3")
                .map(([playerId, playerData]) => {
                  ...
                  return (
                    <div
                      key={playerId}
                      className={`p-4 rounded-xl transition-all duration-300 ${isCurrentHand
                          ? "bg-gradient-to-br from-blue-600/80 to-blue-500/80 text-white shadow-xl border border-blue-400/30"
                          : isActive
                            ? "bg-gradient-to-br from-blue-600/80 to-blue-500/80 text-white shadow-xl border border-blue-400/30"
                            : "bg-gradient-to-br from-red-700/80 to-red-600/80 text-gray-200 border border-red-500/30"
                            ...
                            <div className={`rounded-lg p-2 ${
                              ...
                              <div className="flex space-x-1 mb-1">
                                ...
                                {[...Array(Math.max(0, 2 - (gameState?.players?.[playerId]?.hands?.[0]?.cards?.length ?? 0)))].map((_, index) => (
                                  <div
                                    key={`empty-${index}`}
                                    className={`w-10 h-14 border-2 border-dashed rounded-lg ${isCurrentHand
                                        ? "border-yellow-400/50 bg-yellow-500/10" 
                                        : "border-gray-400 bg-gray-800/50"
                                      }`}
                                  />
                              ...
                              <div className="mt-1 flex items-center justify-between">
                                <span
                                  className={`text-sm font-bold ${(gameState?.players?.[playerId]?.hands?.[0]?.total ?? 0) > 21
                                      ? "text-red-500"
                                      : isCurrentHand
                                        ? "text-gray-900"
                                        : "text-blue-400"
                                ...
                                <div className={`rounded-lg p-2 ${
                                  ...
                                  <div className="flex space-x-1 mb-1">
                                    ...
                                    {[...Array(Math.max(0, 2 - (gameState.players[playerId].split1[0].cards.length ?? 0)))].map((_, index) => (
                                      <div
                                        key={`empty-${index}`}
                                        className={`w-10 h-14 border-2 border-dashed rounded-lg ${isCurrentSplit1Hand
                                            ? "border-yellow-400/50 bg-yellow-500/10"
                                            : "border-gray-400 bg-gray-800/50"
                                          }`}
                                      />
                                  ...
                                  <div className="mt-1 flex items-center justify-between">
                                    <span
                                      className={`text-sm font-bold ${(gameState.players[playerId].split1[0].total ?? 0) > 21
                                          ? "text-red-500"
                                          : isCurrentSplit1Hand
                                            ? "text-gray-900"
                                            : "text-blue-400"
                                ...
                                <div className={`rounded-lg p-2 ${
                                  ...
                                  <div className="flex space-x-1 mb-1">
                                    ...
                                    {[...Array(Math.max(0, 2 - (gameState.players[playerId].split2[0].cards.length ?? 0)))].map((_, index) => (
                                      <div
                                        key={`empty-${index}`}
                                        className={`w-10 h-14 border-2 border-dashed rounded-lg ${isCurrentSplit2Hand
                                            ? "border-yellow-400/50 bg-yellow-500/10"
                                            : "border-gray-400 bg-gray-800/50"
                                          }`}
                                      />
                                  ...
                                  <div className="mt-1 flex items-center justify-between">
                                    <span
                                      className={`text-sm font-bold ${(gameState.players[playerId].split2[0].total ?? 0) > 21
                                          ? "text-red-500"
                                          : isCurrentSplit2Hand
                                            ? "text-gray-900"
                                            : "text-blue-400"
